...
export default function EditSensor(props:any) {
    ...
    const form = useForm<z.infer<typeof formSchema>>({
        ...
        defaultValues: {
          name: props.sensor.getName(),
          model: props.sensor.getModel(),
          local: props.sensor.getLocal(),
          status: props.sensor.getStatus(),
          notes: props.sensor.getNotes()
        },
    })
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        props.sensor.setName(values.name);
        props.sensor.setModel(values.model);
        props.sensor.setLocal(values.local);
        props.sensor.setStatus(values.status);
        props.sensor.setNotes(values.notes);

        try{
            await service.update(props.sensor)
                .then(() => {
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="sm:max-w-[500px] overflow-y-scroll max-h-screen">
                <DialogHeader>
                    <DialogTitle>Editar Usuário {props.sensor.name}</DialogTitle>
